...
def bst_min(t):
    assert t, "t should be a valid tree"
    if t.is_leaf():
        return t.label
    else:
...
def is_bst(t):
    ...
    if t.is_leaf():
        return True
    length = len([b for b in t.branches])   
    if length > 2:
        return False
    if not all([is_bst(b) for b in t.branches]):
        return False
    min_max = [[bst_min(b), bst_max(b)] for b in t.branches]
    if length == 1:
        return min_max[0][0] >= t.label or min_max[0][1] <= t.label
    else:
        return min_max[0][1] <= t.label and min_max[1][0] >= t.label
