...
def missing_digits(n):
    ...
    if n < 10:
        ...
        missing = 0 if last1 <= last2 + 1 else last1 - last2 - 1
...
def count_change(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    def find_max_coin(n, total):
        if n > total:
            return n // 2
        else:
            return find_max_coin(n * 2, total)
    def count_change_with_max_coin(n, m):
        if n < 0 or m < 1:
            return 0
        elif n <= 1 or m == 1:
            return 1
        else:
            return count_change_with_max_coin(n - m, m) + count_change_with_max_coin(n, m // 2)

    return count_change_with_max_coin(total, find_max_coin(1, total))
