...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, attempt):
        nonlocal balance
        if len(attempts) >= 3:
            return "Too many incorrect attempts. Attempts: ['" + attempts[0] + "', '" + attempts[1] + "', '" + attempts[2] + "']"
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    """
    "*** YOUR CODE HERE ***"
    valid_flag = withdraw(0, old_pass)
    if type(valid_flag) == str:
        return valid_flag
    def joint_withdraw(amount, attempt):
        return withdraw(amount, old_pass if attempt == new_pass else attempt)
...
def remainders_generator(m):
    ...
    def remainder_x_generator(x):
        yield from filter(lambda n: n % m == x, naturals())
    yield from map(remainder_x_generator, range(m))
