...
def pingpong(n):
    ...
    def switch(f):
        if f(1, 1) == 2:
            ...
            return add
    def switch_cond(cnt):
        return cnt % 8 == 0 or num_eights(cnt)
    def play_pingpong(val, cnt, func):
        if cnt == n:
            return func(val, 1)
        else:
            if switch_cond(cnt):
                return play_pingpong(func(val, 1), cnt + 1, switch(func))
            else:
                return play_pingpong(func(val, 1), cnt + 1, func)
    return play_pingpong(0, 1, add)
