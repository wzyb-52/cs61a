...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        cnt, place, minr, maxr = 0, self.place, self.min_range, self.max_range
        while cnt != minr and place is not beehive:
            cnt, place = cnt + 1, place.entrance
        while cnt <= maxr and place is not beehive:
            if place.bees:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        # Reduces the armor of the fireant.
        damage_amount, place = amount, self.place
        if amount >= self.armor:
            damage_amount += self.damage
        Ant.reduce_armor(self, amount)
        # Causes damage to all bees in this place.
        cnt, bees = 0, place.bees
        while cnt != len(bees):
            if bees[cnt].armor > damage_amount:
                Insect.reduce_armor(bees[cnt], damage_amount)
                cnt += 1
            else:
                Insect.reduce_armor(bees[cnt], damage_amount) 
        # END Problem 5
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        return self.contained_ant == None and not isinstance(other, ContainerAnt) and isinstance(other, Ant)
        # END Problem 9
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if not self.is_queen:
            ...
            # Inspires all the ants behind the true queen.
            self.throw_at(self.nearest_bee(gamestate.beehive))
...
def test():
    ...
    print("Bee is at {0}".format(bee.place.name))
    print("Time: {0}".format(gamestate.time))
